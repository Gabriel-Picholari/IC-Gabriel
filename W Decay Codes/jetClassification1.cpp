...
void jetClassification1(const char* fileName)
{
    ...
    TH2F * strange_constituentCont = new TH2F("strangeConstituentCont", "Contability of strange tagged constituents per event", 20, 0, 20, 2, 0, 2);
    TH2F * charm_constituentCont = new TH2F("charmConstituentCont", "Contability of charm tagged constituents per event", 20, 0, 20, 2, 0, 2);
    ...
    for ( Long64_t ni = 0; ni < ne; ni++)
    {
        ...
        for (const fastjet::PseudoJet  &jet : tagged_c_jets)
        {
            ...
            Int_t numeroConst = jet.constituents().size();
            for (const fastjet::PseudoJet &constituent : jet.constituents())
            {
                contadorConstituintes++;
                ...
                if (signalType_jet == "charm")
                {
                    charm_constituentCont->Fill(numeroConst, 1);
                }
                else
                {
                    charm_constituentCont->Fill(numeroConst, 0);
                }
            }
        }
        ...
        Int_t contadorConstituintes_s = 0;
        for (const fastjet::PseudoJet &jet : tagged_s_jets)
        {
            ...
            for (const fastjet::PseudoJet &constituent : jet.constituents())
            {
                ...
                if (abs_constituentPdg == 130 || abs_constituentPdg == 310 || abs_constituentPdg == 311 || abs_constituentPdg == 321 || abs_constituentPdg == 313 || abs_constituentPdg == 323 || abs_constituentPdg == 315 || abs_constituentPdg == 325 || abs_constituentPdg == 317 || abs_constituentPdg == 327 || abs_constituentPdg == 319 || abs_constituentPdg == 329 )
                {
                    primary_StrangeRatioHist->Fill(strangeRatio);   // Ideia: o histograma está sendo preenchido multiplas vezes por jato -> Solução potencial: criar um bool aqui e fora do loop de
                }                                                   // constituintes preencher o histograma apenas uma vez por jato.
                else
                {
                ...
                if (signalType_jet == "strange")
                {
                    strange_constituentCont->Fill(numeroConst, 1);
                }
                else
                {
                    strange_constituentCont->Fill(numeroConst, 0);
                }
            }
        }
    ...
    primary_CharmRatioHist->SetLineColor(kBlue);
    primary_CharmRatioHist->Draw("same");
    TLegend *leg = new TLegend(0.6,0.6,0.7,0.7);
    leg->AddEntry(primary_CharmRatioHist, "Com hadron", "l");
    leg->AddEntry(secondary_CharmRatioHist, "Sem hadron", "l");
    leg->Draw("same");
    ...
    primary_StrangeRatioHist->SetLineColor(kBlue);
    primary_StrangeRatioHist->Draw("same");
    leg->Draw("same");

    TCanvas *c4 = new TCanvas("c4", "Contability", 2500, 2500);
    c4->Divide(2, 1);

    c4->cd(1);
    charm_constituentCont->SetTitle("Charm contability");
    charm_constituentCont->GetXaxis()->SetTitle("Code");
    charm_constituentCont->GetYaxis()->SetTitle("Frequency");
    charm_constituentCont->Draw();

    c4->cd(2);
    strange_constituentCont->SetTitle("Strange contability");
    strange_constituentCont->GetXaxis()->SetTitle("Code");
    strange_constituentCont->GetYaxis()->SetTitle("Frequency");
    strange_constituentCont->Draw();
