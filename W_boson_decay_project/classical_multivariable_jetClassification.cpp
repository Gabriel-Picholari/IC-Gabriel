...
void classical_multivariable_jetClassification(const char* fileName, std::string switch_string)
{
    ...
    if (switch_string == "strange") filteredDataFile = new TFile("classical_multivariable_filteredOutput_strange.root", "RECREATE");
    if (switch_string == "charm") filteredDataFile = new TFile("classical_multivariable_filteredOutput_charm.root", "RECREATE");
    ...
    for ( Long64_t ni = 0; ni < ne; ni++)
    {
        ...
        // In this macro, particularly, we'll keep background filling intact and change only signal data. The reasoning behind this is that we confirmed multiple jets classified as strange were, in reality, gluon jets. Thus, to avoid contaminating signal data with gluon jets, specially
        // in the classical analysis, we'll be considering only the tagged jets with most momenta. This way, we make sure this is most likely the main jet from the hard scattering process and, consequently, the one that probably comes undaubtedly from the quark. Since we must keep the
        // same approach for strange and charm tagging, this "one jet assignment" dynamic will be applied to both cases. For more infomation, check the May 26th and mainly May 29th logs in the research log.
        ...
        for (const fastjet::PseudoJet &constituent : maxPtJet.constituents())
        {
        ...
        if (hasSignalConstituent) // Signal data
        {
            ...
            eta = maxPtJet.eta();
            phi = maxPtJet.phi();
            nConst = (Int_t)maxPtJet.constituents().size();
            ...
            signalTree->Fill();
        } // On stand-by for adding an extra else in here (please, refer to the (old) charm only macro for the full comments)
          // Note that I am not throwing the jets that fail the second verification into background! Those are the jets we confirmed to be gluon jets who were emitted by the s shower prior to hadronization and that ended up decaying into a s-sbar quarks and then hadronize.
